...
void LCDDisplay::locate(int column, int row) {
    cursorpos(column+row*0x40);
}

void LCDDisplay::putc(int c) {
    sendeByte(c,0,1);   
}
...
void LCDDisplay::sendeByte(char b,uint8_t rw, uint8_t rs ) {
...
void LCDDisplay::sendeNippel(char b,uint8_t rw, uint8_t rs ) {
...
void LCDDisplay::cursorpos(uint8_t pos) {
...
int LCDDisplay::printf(const char *format, ...) {
    char buf[20];
    va_list args;
